...
function edd_dde_license_page() {
	...
	?>
			<div class="wrap">
				<h2><?php _e('EDD DDE License Options'); ?></h2>
				<form method="post" action="options.php">
				
				<?php settings_fields('edd_dde_license'); ?>
				...
				<table class="form-table">
					<tbody>
						<tr valign="top">	
							...
							<td>
								<input id="edd_dde_license_key" name="edd_dde_license_key" type="text" class="regular-text" value="<?php esc_attr_e( $license ); ?>" />
								<label class="description" for="edd_dde_license_key"><?php _e('Enter your license key'); ?></label>
							</td>
						</tr>
						...
						<tr valign="top">	
							...
							<td>
								<?php if( $status !== false && $status == 'valid' ) { ?>
									...
									<?php wp_nonce_field( 'edd_dde_nonce', 'edd_dde_nonce' ); ?>
		...
		function edd_dde_activate_license() {
			...
			// Call the custom API.
			$response = wp_remote_get( add_query_arg( $api_params, EDD_DDE_STORE_URL ), array( 'timeout' => 15, 'sslverify' => false ) );
